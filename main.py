...
from discord.ui import View, Button, TextInput, select, Select, button
...
@tree.command(name="is_blocked", description="Check if channel is blocked. ")
...
async def _change_windows_loop_policy(interaction: discord.Interaction, windows_loop_policy: app_commands.Choice[str]):
    if str(interaction.user.id) in allowed_ids:
        try:
            if windows_loop_policy.value == "default":
